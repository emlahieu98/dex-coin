      ...
      const temp = dataTimeline.map(item => {
        return {
          ...
          title: item.metadata?.confirm_status
            ? t(
                `accountant.titleHistoryTransaction.withdrawal.${item.metadata?.confirm_status}`,
              )
            : item.metadata?.status
            ? t(
                `accountant.titleHistoryTransaction.withdrawal.${item.metadata?.status}`,
              )
            : '',
        };
      });
